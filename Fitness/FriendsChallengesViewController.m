//
//  FriendsChallengesViewController.m
//  We Get Fit
//
...
#import "FriendsChallengesViewController.h"

@interface FriendsChallengesViewController ()

@end

@implementation FriendsChallengesViewController
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    NSLog (@"FriendsChallengesViewController viewDidAppear");
    
    [self loadObjects];
}

-(void)addWhiteListBlockoutView
{
    NSLog (@"addWhiteListBlockoutView called!");
    
    super.whiteListBlockoutView = [[UIView alloc]initWithFrame:CGRectMake(0, -100, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height*4)];
    [super.whiteListBlockoutView setBackgroundColor:[UIColor whiteColor]];
    [self.view addSubview:super.whiteListBlockoutView];
    
    //Add UITextView asking to Join World Challenge
    UITextView *askToShareData = [[UITextView alloc] init];
    askToShareData = [[UITextView alloc] initWithFrame:CGRectMake(15, [[UIScreen mainScreen] bounds].size.height/3, 275, 275)];
    askToShareData.center = CGPointMake([[UIScreen mainScreen] bounds].size.width/2, [[UIScreen mainScreen] bounds].size.height/2 + 50);
    askToShareData.hidden = NO;
    askToShareData.text = @"Enable 'Friends Challenge' to share and compare your fitness performance with your Facebook friends. This does not post anything to your Facebook wall";
    ...
    [super.whiteListBlockoutView addSubview: askToShareData];
    
    //Add Join button
    UIButton *enableButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [enableButton addTarget:self action:@selector(enableChallenge) forControlEvents:UIControlEventTouchDown];
    [enableButton setTitle:@"Enable" forState:UIControlStateNormal];
    enableButton.frame = CGRectMake(self.view.frame.size.width/4, -150, 160.0, 40.0);
    enableButton.center = CGPointMake([[UIScreen mainScreen] bounds].size.width/2, [[UIScreen mainScreen] bounds].size.height/1.5);
    [super.whiteListBlockoutView addSubview:enableButton];
    
    super.whiteListBlockoutView.userInteractionEnabled = YES;
}
...
{
    NSLog (@"FriendsChallengesViewController queryForTable called!");
        ...
        [userFetched fetchIfNeededInBackgroundWithBlock:^(PFObject *objects, NSError *error)
         {
             ...
             if ([object.objectId isEqualToString:[PFUser currentUser].objectId])
             {
                 ...
                 [healthMethodsSubClass queryTotalNumberOfFacebookFriendsChallengers:^(double done, int yourRank, int totalChallengers, NSError *error)
                  {
